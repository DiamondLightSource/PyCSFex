...
# This code was created by T.Manjo based on the code developed by John P. Sutter et al
# This code creates the basic information of Silicon
...
class Si_isodwf(GeneralCrystal):

    description = "Si, isotropic thermal atomic motion using Debye model"
    ...
    def __init__(self,temperature_K,hkl,energy_eV):
        ...
        self.element_list = ["Si", "Si", "Si", "Si", "Si", "Si", "Si", "Si"]
    ...
    def lattice_unit_cell_params(self,temp_K):
        """Generates lattice parameters for Si"""
             
        alpha_rad = beta_rad = gamma_rad = np.pi/2

        a_lattice_A = self.calc_lattice_para_Si(temp_K)
    ...
    # It uses a 5-term fit of the same form as the 3-term fit by Middelmann, et al.
    # the additional 2 terms provide slightly better agreement at higher temperature
    # Manjo start
    def calc_lattice_para_Si(self, temp_K):
        #data(<300) from T. Middelmann, et al.; PRB 92, 174113 (2015)
        #data(300<) from Y. Okada, et al.; JAP 56, 2 (1984)
        #Manjo fit the all data with middlemann's function (eq.(5) in T. Middelmann, et al.; PRB 92, 174113 (2015))
        
        a_i = [-5.54856E-06, 2.72076E-05, 2.90700E-06, 2.03094E-04, 7.31462E-04]    #m/K
        theta_i = [2.06418E+02, 6.27801E+02, 1.80632E+03, 1.63626E+04, 1.63683E+04] #K
        l_0 = 5.429822  #Angstrom
    ...
    def refatom_coordinates(self):
        ...
        Si_refatom_pos_1 = np.array(["Si", 0.125, 0.125, 0.125])
        Si_refatom_pos_2 = np.array(["Si", 0.875, 0.375, 0.375])

        refatom_coords = np.array([Si_refatom_pos_1, Si_refatom_pos_2])
        
        Check.length(Si_refatom_pos_1,4)
        Check.length(Si_refatom_pos_2,4)
    ...
    def atoms_init_and_update(self,Initial):        
        ...
        # Manjo_start
        """calculate atom coordination of Si (Diamond structure, Space Group:Fd-3m(No.227 Origin Choice 2))"""
        #initialize
        coord_list = []
        M_and_TD_Atom = []
        ...
        for i in range(np.shape(coords_ori)[0]):  #loop refatom pos
            for j in range(4):  #loop shift vector
                Si_coord_c = coords_ori[i] + Shift_vector[j]
                Si_coord_c = np.where(Si_coord_c >= 1.0, Si_coord_c - 1.0, Si_coord_c)  #0 <= atom coordination < 1.0 
                Si_coord_c = np.where(Si_coord_c < 0.0, Si_coord_c + 1.0, Si_coord_c)
                coord_list.append(Si_coord_c)
                M_and_TD_Atom.append([Si_M_amu,Si_T_Debye_K])
        # Manjo_end
